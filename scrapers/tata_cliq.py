"""
Tata Cliq Daily Deals Scraper
Scrapes deals from Tata Cliq's deals page
"""
...
class TataCliqScraper:
    """Scraper for Tata Cliq daily deals"""
    
    BASE_URL = "https://www.tatacliq.com"
    DEALS_URL = "https://www.tatacliq.com/offers"
    
    def __init__(self):
        self.website_name = "Tata Cliq"
    
    def scrape_deals(self, max_deals: int = 50) -> List[Dict]:
        """
        Scrape daily deals from Tata Cliq
        ...
        try:
            ...
            deal_containers = soup.find_all('div', class_=re.compile(r'ProductModule|ProductCard'))
            
            if not deal_containers:
                # Alternative selector
                deal_containers = soup.find_all('article')
    ...
    def _extract_deal_from_container(self, container) -> Optional[Dict]:
        ...
        try:
            # Product name
            name_elem = container.find(['h3', 'h4', 'div'], class_=re.compile(r'ProductTitle|title'))
            if not name_elem:
            ...
            # Brand
            brand_elem = container.find(['span', 'div'], class_=re.compile(r'ProductBrand|brand'))
            brand = clean_text(brand_elem.get_text()) if brand_elem else self._extract_brand_from_name(product_name)
            
            # Prices
            price_elem = container.find(['span', 'div'], class_=re.compile(r'ProductPrice|price'))
            original_price_elem = container.find(['span', 'div'], class_=re.compile(r'striked'))
            ...
            discount_elem = container.find(['span', 'div'], class_=re.compile(r'DiscountOff|discount'))
            if discount_elem:
...
def scrape_tata_cliq_deals(max_deals: int = 50) -> List[Dict]:
    """
    Main function to scrape Tata Cliq deals
    ...
    Returns:
        List of deal dictionaries
    """
    scraper = TataCliqScraper()
    return scraper.scrape_deals(max_deals)


if __name__ == "__main__":
    # Test the scraper
    deals = scrape_tata_cliq_deals(max_deals=10)
