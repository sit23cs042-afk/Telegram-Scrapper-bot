...
class AjioScraper:
    """Scraper for Ajio daily deals"""
    
    BASE_URL = "https://www.ajio.com"
    
    def __init__(self):
        self.website_name = "Ajio"
    
    def scrape_deals(self, max_deals: int = 50) -> List[Dict]:
        """
        Scrape deals from Ajio using browser automation
        ...
        category_urls = [
            ...
            "https://www.ajio.com/men-shirts/c/830216003",
            "https://www.ajio.com/women-dresses/c/830201047"
        ]
        ...
        try:
            with BrowserHelper() as browser:
                for category_url in category_urls:
                    ...
                    # Fetch page with browser and scroll to load products
    ...
    def _extract_deal_from_link(self, link_elem) -> Optional[Dict]:
        ...
        try:
            ...
            # Product name
            name_elem = container.find(['div', 'span', 'p'], class_=re.compile(r'name|brand|product', re.I))
            ...
            # Prices
            price_elems = container.find_all(['span', 'div', 'strong'], class_=re.compile(r'price', re.I))
    ...
    def _extract_deal_from_container(self, container) -> Optional[Dict]:
        ...
        try:
            # Product name
            name_elem = container.find(['div', 'span'], class_=re.compile(r'nameCls|name'))
            if not name_elem:
            ...
            # Brand
            brand_elem = container.find(['div', 'span'], class_=re.compile(r'brand'))
            brand = clean_text(brand_elem.get_text()) if brand_elem else self._extract_brand_from_name(product_name)
            
            # Prices
            price_elem = container.find(['span'], class_=re.compile(r'price'))
            original_price_elem = container.find(['span'], class_=re.compile(r'orginal-price'))
            ...
            discount_elem = container.find(['div', 'span'], class_=re.compile(r'discount'))
            if discount_elem:
...
def scrape_ajio_deals(max_deals: int = 50) -> List[Dict]:
    """
    Main function to scrape Ajio deals
...
if __name__ == "__main__":
    # Test the scraper
    deals = scrape_ajio_deals(max_deals=10)
